...
static std::pair<int, int> pickSeeds(const std::vector<EntryType>& entries) {
   ...
   // std::cout << "[pickSeeds] Starting seed selection for " << total << " entries\n";
    ...
    //std::cout << "[pickSeeds] Chosen seeds: " << seed1 << " and " << seed2
              //<< " with worst waste: " << worstWaste << "\n";
...
static void assignEntryToNode(std::shared_ptr<RTreeNode> node, const EntryType& entry) {
    if constexpr (std::is_same_v<EntryType, std::pair<BoundingBox3D, std::shared_ptr<Trajectory>>>) {
        node->insertLeaf(entry.first, entry.second);
       // std::cout << "[assignEntryToNode] Assigned leaf entry with volume "
                //  << entry.first.volume() << "\n";
    } else if constexpr (std::is_same_v<EntryType, std::pair<BoundingBox3D, std::shared_ptr<RTreeNode>>>) {
        node->insertChild(entry.first, entry.second);
        //std::cout << "[assignEntryToNode] Assigned internal entry with volume "
                 // << entry.first.volume() << "\n";
    }
}
                               ...
                               int minFill) 
{
    ...
    if (leftSize + remaining <= minFill) {
       // std::cout << "[assignForcedEntries] Forcing " << remaining
                  //<< " entries to left node to satisfy minFill\n";
        for (int i = 0; i < total; ++i) {
            if (!assigned[i]) {
    ...
    if (rightSize + remaining <= minFill) {
       // std::cout << "[assignForcedEntries] Forcing " << remaining
                 // << " entries to right node to satisfy minFill\n";
        for (int i = 0; i < total; ++i) {
            if (!assigned[i]) {
                         ...
                         bool& assignToLeft) 
{
    ...
    if (next != -1) {
      //  std::cout << "[pickNextEntry] Chosen entry " << next
         //         << " with enlargement diff " << maxDiff
           //       << " assigned to " << (assignToLeft ? "left" : "right") << "\n";
    }
    return next;
}
                                  ...
                                  int maxEntries) 
{
   ...
   // std::cout << "[quadraticSplitEntries] Starting quadratic split with maxEntries " << maxEntries << "\n";
  ...
  //  std::cout << "[quadraticSplitEntries] Finished split. Left MBR volume: "
           //   << leftNode->getMBR().volume()
            //  << ", Right MBR volume: " << rightNode->getMBR().volume() << "\n";
}
