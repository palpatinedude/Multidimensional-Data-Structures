# This code reads trajectory data from a CSV file from range query,
# groups the data by trajectory ID, and visualizes each trajectory
# in a 3D plot with optional bounding boxes using Plotly.
...
# ---------------- Load CSV ----------------
csv_file = "../results/query_1_rangeQuery_plot.csv" 
...
for tid, traj in trajectories.items():
    ...
    # ---- Compute bounding box from point data ----
    minX, maxX = traj['X'].min(), traj['X'].max()
    minY, maxY = traj['Y'].min(), traj['Y'].max()
    minT, maxT = traj['T'].min(), traj['T'].max()

    # Define 12 edges of the box (bottom, top, vertical edges)
    box_edges = [
