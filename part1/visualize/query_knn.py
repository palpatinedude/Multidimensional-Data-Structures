...
# from its k-nearest neighbor trajectories, and visualizes all trajectories 
# in a 3D plot using Plotly.
...
# ---------------- Load CSV ----------------
df = pd.read_csv("../results/query_2_kNN_plot.csv")  # Path to your CSV file

# ---------------- Separate trajectories ----------------
# Query trajectory
query_df = df[df['Type'] == 'query']

# Neighbor trajectories (k-nearest neighbors)
...
for i, traj_id in enumerate(neighbor_ids):
    traj_points = result_df[result_df['TrajectoryID'] == traj_id]  # Select points for this neighbor
    fig.add_trace(go.Scatter3d(
        x=traj_points['X'],
        y=traj_points['Y'],
        z=traj_points['T'],
...
# ---------------- Layout settings ----------------
fig.update_layout(
    title="k-Nearest Neighbors Trajectories (3D)",
    scene=dict(
